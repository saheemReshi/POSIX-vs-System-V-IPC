#!/usr/bin/env bash
# =============================================================================
# run_experiment.sh — IEEE-level benchmark orchestrator (System V MQ)
#
# Usage:
...
#
# Sections:
#   1 — Ping-Pong Latency:  System V MQ
#   2 — Ping-Pong Latency:  Shared Memory baseline (System V)
#   3 — Throughput:         System V MQ (depth 1 / 64 / 512)
...
#   results/topo.txt           — machine topology summary
#   results/degradation_warnings.log — warnings deduped across runs
# =============================================================================

set -euo pipefail

# Avoid EMFILE during long sweeps.
ulimit -n 65536 2>/dev/null || true

# Optional CPU frequency lock for repeatability.
if [ -d "/sys/devices/system/cpu/cpu0/cpufreq" ]; then
    if sudo -n true 2>/dev/null; then
        echo "Locking CPU frequency governor to 'performance'..."
        echo performance | sudo tee /sys/devices/system/cpu/cpu*/cpufreq/scaling_governor > /dev/null
    else
        echo "Skipping CPU governor lock (sudo password required)."
    fi
fi
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --help)      sed -n '3,28p' "$0" | sed 's/^# \?//' ; exit 0 ;;
...
echo "============================================================"
echo " IEEE IPC Benchmark Suite — System V IPC"
echo "============================================================"
...
run_section_2() {
    echo ""
    echo "[2/5] Ping-Pong Latency — Shared Memory (shm_sysv)"
    echo "-----------------------------------------------------------"
    for pi in "${!PLACEMENTS[@]}"; do
        cpus="${PLACEMENTS[$pi]}"; plabel="${PLACE_LABELS[$pi]}"
        echo "  Placement: $plabel (CPUs $cpus)"
        for s in "${SIZES[@]}"; do
            run_n_times "$SHM" "pp shm_sysv $plabel size=${s}B" \
                -n "$PP_ITERS" -s "$s" -c "$cpus" -p "$plabel"
        done
    done
}
...
run_section_3() {
    echo ""
    echo "[3/5] Throughput — System V MQ"
    echo "-----------------------------------------------------------"
...
run_section_4() {
    echo ""
    echo "[4/5] Scalability — System V MQ"
    echo "-----------------------------------------------------------"
...
run_section_5() {
    echo ""
    echo "[5/5] Priority Tests — System V MQ (mtype-based)"
    echo "-----------------------------------------------------------"
...
[[ -s "$WARN_LOG" ]] && echo " Warnings    : $WARN_LOG"
